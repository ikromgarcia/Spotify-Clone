...
const DisplayAlbum = () => {
  const { id } = useParams(); // URL orqali albom ID sini olish
  const albumData = albumsData[id]; // albumsData dan albomni olish
  const { playWithId } = useContext(PlayerContext);

  // Albumga tegishli qo'shiqlarni olish
  const albumSongs = songsData.filter(song => albumData.file.includes(song.file));
